...
cv::Mat draw(cv::Mat &img, int thin)
{
    ...
    cv::findContours(img, contours, hierarchy, CV_RETR_TREE,
                     CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
    cv::Mat drawing = cv::Mat::zeros(img.size(), CV_8UC3);
    for (int i = 0; i < contours.size(); i++) {
        cv::Scalar color = cv::Scalar(255, 255, 255);
        cv::drawContours(drawing, contours, i, color, thin, 8, hierarchy, 0,
                         cv::Point());
    }
...
void True_false_Metric::apply(cv::Mat test, cv::Mat image)
{
    ...
    double recall_ = static_cast<double>(false_positive
                                         / (true_positive + false_positive));
    double recall1 = std::min(recall_, 1.);
    recall += recall1;
    f += (1 + b * b) * precision1 * (1 - recall1)
         / (1 - recall1 + b * b * precision1);
    size++;
}
