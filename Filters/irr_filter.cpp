...
#include <iostream>
IRR_filter_Imlementation::IRR_filter_Imlementation(Mat image, float error)
...
Mat trac(cv::Mat src)
{
    ...
    cv::findContours(src, contours, hierarchy, CV_RETR_EXTERNAL,
...
Mat IRR_filter_Imlementation::proc()
{
    ...
    while (dE > eps && i < max_iter_L) {
        ...
        /// 3.d)-----------------compute magnutude of zero crossing in error
        /// signal zce(x,y)  -------------------------
        calculateZCE();
        /// 4)------------------ update edges
        /// ------------------------------------------
    ...
    while (dU > eps && i < max_iter_U) {
        for (int y = begin; y < U_min.rows - 1; y++) {
            ...
            for (int x = begin; x < U_min.cols - 1; x++) {
                float T = B_row[x] + L_h_row_y[x] + L_h_row_y_1[x] + L_v_row[x]
                          + L_v_row[x + 1];
                float a = (L_h_row_y[x] * U_min_row[x]
                           + L_h_row_y_1[x] * U_n.at<float>(y + 1, x)
                           + L_v_row[x] * U_min_row1[x - 1]
                           + L_v_row[x + 1] * U_min_row1[x + 1]);
                U_min_row1[x]
                    = U_n_row[x]
                      - q * (T * U_n_row[x] - B_row[x] * image_row[x] - a) / T;
            }
        }
...
{
    cv::GaussianBlur(error_abs, control_signal, cv::Size(3, 3), 0, 0,
                     cv::BORDER_DEFAULT);
    cv::subtract(control_signal, cv::Scalar_<float>(sigma), control_signal);
    cv::GaussianBlur(control_signal, control_signal, cv::Size(25, 25), 0, 0,
                     cv::BORDER_DEFAULT);
}
void IRR_filter_Imlementation::calculateZCE()
{
    ...
    for (int y = 1; y < image.rows - 1; y++) {
        ...
        for (int x = 1; x < image.cols - 1; x++) {
            float shr
                = (error_row1[x + 1] + error_row2[x + 1] + error_row3[x + 1])
                  / 3;
            float shl
                = (error_row1[x - 1] + error_row2[x - 1] + error_row3[x - 1])
                  / 3;
            float sht
                = (error_row3[x + 1] + error_row3[x] + error_row3[x - 1]) / 3;
            float shb
                = (error_row1[x + 1] + error_row1[x] + error_row1[x - 1]) / 3;
            float zce_h = 0.;
            float zce_v = 0.;
            if ((shr * shl) < 0) {
...
void IRR_filter_Imlementation::updateLambda()
{
    for (int y = 0; y < L.rows; y++) {
        ...
        for (int x = 0; x < L.cols; x++) {
            if (control_signal_row[x] > 0 && zce_row[x] > tresh
                && L_row[x] > l_min) {
